...
def lives_dictionary(callsdf):
    ...
    for ego in callsdf['ego'].unique():
        result[ego] = {}
        df1 = callsdf.loc[callsdf['ego'] == ego]
        for alter in df1['alter'].unique():
            result[ego][alter] = {}
            df2 = df1.loc[df1['alter'] == alter]
            df2 = df2.sort_values(by='aclock', ascending=True)
            first = list(df2.index)[0]
            last = list(df2.index)[-1]
            result[ego][alter]['t0'] = df2.at[first, 'uclock']
            result[ego][alter]['tf'] = df2.at[last, 'uclock']
            result[ego][alter]['ell'] = result[ego][alter]['tf'] - result[ego][alter]['t0']
            result[ego][alter]['nij'] = len(df2)
    return result
...
def get_f(callsdf, theego, lives_dict, bina, binell):
    ...
    for ego in df1['ego'].unique():
        ...
        for alter in df2['alter'].unique():
            ...
            df3.sort_values(by='alpha', inplace=True)
            for alpha in df3['alpha'].unique():
                df4 = df3.loc[df3['alpha'] == alpha]
                f[ego][alter] = len(df4)
...
def plot_g(gresult, lives_dict, xaxis, binell):
    result = {}
